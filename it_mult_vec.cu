...
void mult_vec_shared_x(int n, int rows_per_thread, float *y, float *d, float *A,
  ...
  for (int i = 0; i < copies_per_thread; i++) {
    int copy_index = idx * rows_per_thread + i;//Change this line based on the responsibility of this thread to fetch part of x
...
int it_mult_vec(int N,
  ...
  while (k < iterations) {
    if (use_async) {
      ...
      /*Your solution*/
        mult_vec_shared_x<<<num_blocks, threads_per_block>>>(
